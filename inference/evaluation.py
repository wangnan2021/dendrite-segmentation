...
class measure(object):
    ...
    def f_measure(self, truth, pred, F_alpha=None):
        ...
        precision = TP.sum() / pred.sum() if pred.sum(
        ) != 0 else 0  # precision = TP / (TP + FP)
        recall = TP.sum() / truth.sum() if truth.sum(
        ) != 0 else 0  # recall = TP / (TP + FN)
        f_measure = (alpha**2 + 1) * precision * recall / (
            alpha**2 * (recall + precision)) if recall + precision != 0 else 0
        return {
            'accuracy': accuracy,
            'precision': precision,
            'recall': recall,
...
class mydataset(Dataset):
    def __init__(self, base_path, is_tta=False, mode='test'):
        ...
        print("The length of mydataset_" + mode +
              " is %d" % len(self.img_name))
    ...
    def __getitem__(self, index):
        ...
        mask = cv2.imread(
            os.path.join(os.path.dirname(self.base_path), "mask",
        ...
        if self.is_tta:
            ...
            return augmented['image'], image_name, albu.Compose(
                [albu.Normalize(), AT.ToTensor()])(image=image)['image'], mask
        else:
            augmented = albu.Compose([albu.Normalize(),
                                      AT.ToTensor()])(image=image)
...
if __name__ == '__main__':
    ...
    if ensemble:
        ...
        if TTA:
            for i in range(len(os.listdir(load_path))):
                param = torch.load(
                    os.path.join(load_path,
                                 'checkpoint_epoch' + str(i) + '.pth.tar'))
            ...
            for index, (image, image_name, notta_image,
                        ori_mask) in enumerate(tqdm(dataset)):
                ...
                for net in models:
                    with torch.no_grad():
                        ...
                        mask_pre_notta = torch.sigmoid(
                            mask_pre_notta.detach()).cpu().numpy().squeeze()
                    mask_pre = (cv2.flip(mask_pre, 1) + mask_pre_notta) / 2
                    mask += mask_pre
                mask /= len(models)
                cv2.imwrite(os.path.join(OUT_PATH, image_name[0] + '.png'),
                            ((mask > 0.5) * 255).astype(np.uint8))
                measure_dict = measure.total_measure(
                    (ori_mask.detach().numpy().squeeze()) / 255, mask)
            ...
            log.write(
                'pixel_acc     | precision        | recall        | iou         | f_score     | MAE_score \n'
            )
            log.write(
                '---------------------------------------------------------------------------------------\n'
            )
            log.write(
                ...
                % (result['pixel_acc'], result['precision'], result['recall'],
                   result['iou'], result['f_score'], result['MAE_score']))
            print("done!!!")
        else:
            for i in range(len(os.listdir(load_path))):
                param = torch.load(
        ...
        if TTA:
            ...
            for index, (image, image_name, notta_image,
                ...
                cv2.imwrite(os.path.join(OUT_PATH, image_name[0] + '.png'),
                            (tta_result * 255).astype(np.uint8))
                measure_dict = measure.total_measure(
                    (ori_mask.detach().numpy().squeeze()) / 255, tta_result)
                pixel_acc += measure_dict["accuracy"]
                precision += measure_dict['precision']
                recall += measure_dict['recall']
                iou += measure_dict['iou']
            ...
            for index, (image, image_name,
                ...
                with torch.no_grad():
                    ...
                    mask_pre = mask_pre.cpu().numpy().squeeze()
                cv2.imwrite(
                    os.path.join(OUT_PATH, image_name[0].replace('tif',
                                                                 'tif')),
                    ((mask_pre > 0.5) * 255).astype(np.uint8))
                cv2.imwrite(
                    os.path.join(OUT_PATH, image_name[0].replace('tif',
                                                                 'png')),
                    ((mask_pre) * 255).astype(np.uint8))
                measure_dict = measure.total_measure(
                    (ori_mask.detach().numpy().squeeze()) / 255, mask_pre)
                pixel_acc += measure_dict["accuracy"]
                precision += measure_dict['precision']
                recall += measure_dict['recall']
                iou += measure_dict['iou']
