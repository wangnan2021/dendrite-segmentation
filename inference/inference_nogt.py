...
class mydataset(Dataset):
    def __init__(self, base_path, is_tta=False, mode='test'):
        ...
        self.image_list = [
            os.path.join(base_path, img) for img in os.listdir(base_path)
        ]
        print("The length of mydataset_" + mode +
              " is %d" % len(self.image_list))
    ...
    def __getitem__(self, index):
        ...
        if self.is_tta:
            ...
            return augmented['image'], image_name, albu.Compose(
                [albu.Normalize(), AT.ToTensor()])(image=image)['image']
        else:
    ...
    for instance in IMG_LIST:
        ...
        if ensemble:
            for step in range(5):
                ...
                if TTA:
                    pseudolabels = {}
                    mydataset_test_TTA = mydataset(IMG_PATH,
                                                   is_tta=True,
                                                   mode="test")
                    dataset_tta = DataLoader(mydataset_test_TTA,
                                             batch_size=1,
                                             shuffle=False)
                    # print("!!!!!")
                    for index, (image,
                                image_name) in enumerate(tqdm(dataset_tta)):
                        with torch.no_grad():
                            mask_pre = model(image.cuda())
                            mask_pre = torch.sigmoid(mask_pre.detach())
                            mask_pre = mask_pre.cpu().numpy().squeeze()
            ...
            for i in range(len(dataset)):
                confidence = (np.sum(overall_pred[i] < 0.2) +
                              np.sum(overall_pred[i] > 0.8)) / (h * w)
                name = null_name[i]
            ...
            if TTA:
                ...
                for index, (image, image_name,
                            notta_image) in enumerate(tqdm(dataset)):
                    with torch.no_grad():
                        mask_pre = model(image.cuda())
                        mask_pre = torch.sigmoid(mask_pre.detach())
                        mask_pre = mask_pre.cpu().numpy().squeeze()
                        ...
                        cv2.imwrite(
                            os.path.join(OUT_PATH,
                                         image_name[0].split('.')[0] + ".png"),
                            ((tta_result > 0.5) * 255).astype(np.uint8))
