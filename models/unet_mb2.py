...
class Decoderv1(nn.Module):
    ...
    def forward(self, x, e=None, shape=None):
        if e is not None:
            x = F.upsample(input=x, size=(e.size(2), e.size(3)), mode='bilinear', align_corners=False)
            x = torch.cat([x, e], 1)
        else:
            x = F.upsample(input=x, size=(shape[0], shape[1]), mode='bilinear', align_corners=False)
        x = self.conv1(x)
        x = self.conv2(x)
        return x
...
class DeepLabV3MobileNetV2(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.down1 = nn.Sequential(
            *(get_inverted_residual_blocks(self.c[0], self.c[1], t=self.t[1], s=self.s[1],
                                         n=self.n[1]) +
            get_inverted_residual_blocks(self.c[1], self.c[2], t=self.t[2], s=self.s[2],
                                         n=self.n[2])
        ))
        self.down2 = get_inverted_residual_blocks(self.c[2], self.c[3], t=self.t[3], s=self.s[3],
                                             n=self.n[3])[0]

        self.down3 = get_inverted_residual_blocks(self.c[3], self.c[4], t=self.t[4], s=self.s[4],
                                             n=self.n[4])[0]

        self.down4 = nn.Sequential(
            *(get_inverted_residual_blocks(self.c[4], self.c[5], t=self.t[5], s=self.s[5],
                                         n=self.n[5]) +
            get_inverted_residual_blocks(self.c[5], self.c[6], t=self.t[6], s=self.s[6],
                                         n=self.n[6])
        ))
...
class DeepLabV3MobileNetV2_V2(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.logit = nn.Sequential(nn.Conv2d(160, 32, kernel_size=3, padding=1),
                                   nn.ELU(True),
    ...
    def forward(self, x): # x.size = 3,256,256
        ...
        f = torch.cat(
            (
                e1,
                F.upsample(e2, size=(e1.size(2), e1.size(3)), mode='bilinear', align_corners=False),
                F.upsample(e3, size=(e1.size(2), e1.size(3)), mode='bilinear', align_corners=False),
                F.upsample(e4, size=(e1.size(2), e1.size(3)), mode='bilinear', align_corners=False),
                F.upsample(e5, size=(e1.size(2), e1.size(3)), mode='bilinear', align_corners=False)),1)
        out = self.logit(f)
        return out
