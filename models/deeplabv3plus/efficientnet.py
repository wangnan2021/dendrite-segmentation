...
class EfficientNetPartial(EfficientNet):
    def __init__(self, *args, **kwargs):
        intermediate = kwargs.pop('intermediate', None)
        super(EfficientNetPartial, self).__init__(*args, **kwargs)
        # self.intermediate = {
        #     'b0':[2, 10],
        #     'b1':[4, 15],
        #     'b7':[10, 37]
        ...
        for idx in range(max(self.intermediate) + 1, self.num_blocks):
            self._blocks[idx] = None
        self._conv_head = None
        self._bn1 = None
        self._fc = None
    ...
    def forward(self, x):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            drop_connect_rate = self._global_params.drop_connect_rate
            if drop_connect_rate:
                drop_connect_rate *= float(idx) / self.num_blocks
            x = block(x, drop_connect_rate=drop_connect_rate)
            if idx in self.intermediate: self.layers.append(x)
