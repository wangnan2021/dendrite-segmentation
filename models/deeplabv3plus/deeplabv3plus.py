...
class deeplabv3plus(nn.Module):
    def __init__(self,
        ...
        if decoder_name.find('denseaspp') >= 0:
            self.aspp = DenseASPP(in_channels=indim[-1],
                                  nclass=256,
                                  conv_layer=conv)
        ...
        self.shortcut_conv = nn.Sequential(
            ...
            nn.BatchNorm2d(48, momentum=0.9), nn.ReLU(inplace=True))
        if aux:
            self.aux_conv = nn.ModuleList()
            for idx in range(3):
                self.aux_conv.append(
                    nn.Sequential(
                        conv(indim[idx], 48, 1, 1, padding=0, bias=True),
                        nn.BatchNorm2d(48,
    ...
    def forward(self, x):
        ...
        feature_aspp = F.upsample(feature_aspp,
                                  size=(feature_shallow.size(2),
                                        feature_shallow.size(3)),
                                  mode='bilinear',
                                  align_corners=True)
        aux = None
        if hasattr(self, 'aux_conv'):
            aux = {}
            for idx in range(3):
                aux['aux{}'.format(idx)] = F.interpolate(self.aux_conv[idx](
                    layers[idx]),
                                                         size=x.shape[-2:],
                                                         mode='bilinear',
                                                         align_corners=True)
...
def deeplabv3plus_efficientnet(version='b7',
                               decoder_name='aspp',
                               aux=False,
                               pretrained=False):
