...
class BatchRename():
    ...
    def rename(self):
        ...
        for item in filelist:
            if item.endswith('.nrrd'):  #初始的图片的格式为jpg格式的（或者源文件是png格式及其他格式，后面的转换格式就可以调整为自己需要的格式即可）
                fnrrd = os.path.join(os.path.abspath(self.path), item)
                #print(fnrrd)
                new = item[:-5]
                src = os.path.join(os.path.abspath(self.sourcepath), (new + '.tif'))
                dst = os.path.join(os.path.abspath(self.maskpath), (new + '.png'))#处理后的格式也为jpg格式的，当然这里可以改成png格式
                ...
                try:
                    ...
                    img = cv2.imread(src)
                    imgh = cv2.flip(img, 1)
                    src0 = os.path.join(os.path.abspath(self.imagepath), (new + '.png'))
                    cv2.imwrite(src0,img)
                    srch = os.path.join(os.path.abspath(self.imagepath), (new + '_h.png'))
                    cv2.imwrite(srch,imgh)

                    imgv = cv2.flip(img, 0)
                    srcv = os.path.join(os.path.abspath(self.imagepath), (new + '_v.png'))
                    cv2.imwrite(srcv,imgv)

                    imghv = cv2.flip(img, -1)
                    srchv = os.path.join(os.path.abspath(self.imagepath), (new + '_hv.png'))
                    cv2.imwrite(srchv,imghv)

                    maskh = cv2.flip(mask, 1)
                    dsth = os.path.join(os.path.abspath(self.maskpath), (new + '_h.png'))
                    cv2.imwrite(dsth,maskh)

                    maskv = cv2.flip(mask, 0)
                    dstv = os.path.join(os.path.abspath(self.maskpath), (new + '_v.png'))
                    cv2.imwrite(dstv,maskv)

                    maskhv = cv2.flip(mask, -1)
                    dsthv = os.path.join(os.path.abspath(self.maskpath), (new + '_hv.png'))
